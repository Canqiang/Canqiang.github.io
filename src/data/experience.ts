...
export const experience: ExperienceEntry[] = [
  ...
  },
  {
    ...
    summary: {
      en: 'Core contributor to Core-AI, a team-built open-source AI Agent framework integrating an SDK, terminal agent, self-hosted server, and web interface, alongside agent engineering for a restaurant-industry menu AI product and, most recently, ownership of SEO agent evaluation workflows.',
      zh: 'Core-AI 核心贡献者。Core-AI 是由团队共同建设的开源 AI Agent 框架，集成 SDK、终端 Agent、自托管服务端与 Web 界面；同时负责餐饮行业菜单 AI 产品的智能体工程，近期开始牵头 SEO 智能体评估流程。',
    },
    highlights: {
      en: [
        'Designed and shipped durable agent workflow capabilities — human-in-the-loop steps, nested workflows, run resumption, artifact delivery, and execution tracing — with the core workflow engine validated through two review cycles and 30 targeted tests ahead of Dev-environment sign-off.',
        'Built model-gateway and multi-provider runtime integrations across OpenAI-compatible and Responses APIs, strengthened by session recovery, observability, and sandbox snapshot and restore.',
        'Delivered agent capabilities for menu understanding, data normalization, content completion, and menu optimization on a restaurant-industry AI product, working through inconsistent source data, evolving business rules, and unstable model outputs to ship into an existing product flow.',
        'Took ownership of SEO agent evaluation workflows, consolidating existing agents, skills, reports, and data pipelines into a repeatable evaluation methodology ahead of real-customer pilots.',
        'Shipped agent capabilities end to end — from algorithm and workflow logic through backend services, user-facing interfaces, and cloud runtime — rather than handing off model work at the prototype stage.',
      ],
      zh: [
        '设计并交付智能体工作流的关键能力——人工介入、嵌套工作流、运行恢复、产物交付与执行追踪——核心工作流引擎经过两轮评审、通过 30 项定向测试后完成 Dev 环境验收。',
        '建设模型网关与多供应商运行时集成，覆盖 OpenAI 兼容接口与 Responses API，并通过会话恢复、可观测链路与沙箱快照恢复提升可靠性。',
        '为餐饮行业菜单 AI 产品交付菜单理解、数据处理、内容补全与菜单优化等智能体能力，处理不统一的源数据、多变的业务规则与不稳定的模型输出，并将结果接入现有产品流程。',
        '接手 SEO 智能体评估流程的负责工作，将现有 Agent、Skill、报告与数据流程梳理整合为一套可执行的评估方法，为后续真实客户试点做准备。',
        '从算法与工作流逻辑，到后端服务、用户交互界面与云端运行环境，端到端交付智能体能力，而非把模型工作停留在原型阶段就转手。',
      ],
    },
  },
];
